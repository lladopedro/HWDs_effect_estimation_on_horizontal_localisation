% demo_llado2021 demonstrates the use of the llado2021 model. The detailed
...
%% Choose option:
precomputedModel = 0;   % 0: compute ITD and ILD estimates and train model;
...
if precomputedModel == 1
    ...
    y_hat = llado2021_evaluateNN(x_test,NN_pretrained);    
    y_est_dir = y_hat(:,1);
...
if precomputedModel == 0
    ...
    for file_id = 1:nDevices
        ...
        ir_sofa = llado2021_extractIRs(sofaFile_name,train_lateral_angles,fs);
        if file_id == 1
            ir = ir_sofa;
        else
            ir = [ir;ir_sofa];
    ...
    [x_train] = llado2021_binauralFeats(ir,stim,fs);
...
%         train_lateral_angles,stim,fs,1);
    ...
    test_ir = llado2021_extractIRs(sofaFile_name_test,train_lateral_angles,fs);
    x_test = llado2021_binauralFeats(test_ir,stim,fs);
    ...
    for iter = 1:nIter
        %% Train NN: direction
        [net_dir,tr_dir] = llado2021_trainNN(x_train',y_train(:,1),hiddenLayerSize_dir,augmentation_ratio,SNR);

        %% Train NN: uncertainty
        [net_uncertainty,tr_uncertainty] = llado2021_trainNN(x_train',y_train(:,2),hiddenLayerSize_uncertainty,augmentation_ratio,SNR);
...
%%
if exist('y_test') == 1
    llado2021_plotEstimation(y_est_dir,y_est_uncertainty,angle_id,y_test);
else
    llado2021_plotEstimation(y_est_dir,y_est_uncertainty,angle_id);
end
...
if precomputedModel == 1
    showWeights = 1;
    if showWeights == 1      
        llado2021_weightsAnalysis(NN_pretrained);
    end
end