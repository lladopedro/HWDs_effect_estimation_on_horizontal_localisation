function [y_est] = llado2021(ir,stim,fs,NN_pretrained)
%LLADO2021 Binaural perceptual similarity
%
%   Input parameters:
...
%                        'demo_llado2021'
%
%   LLADO2021(...) is a model for estimating the effect of head-worn
...
%   script 'demo_llado2021' the whole procedure.
%
%
%   Authors:
...
%   Correspondence to pedro.llado@aalto.fi
    ...
    if nargin<4; load('NN_pretrained.mat'); end % Find an example on how to train the network in 'demo_llado2021'
    if nargin<3; fs=48000; end
    if nargin<2; stim = pinknoise(0.25*fs); end
    
    %% EXTRACT BINAURAL FEATURES
    binauralFeatures = llado2021_binauralFeats(ir,stim,fs);
    
    %% EVALUATE PRETRAINED NETWORK
    y_est = llado2021_evaluateNN(binauralFeatures,NN_pretrained);
end